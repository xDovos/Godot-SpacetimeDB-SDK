...
func build_gdscript_from_schema(schema: Dictionary) -> Array[String]:
	...
	var output_file_path: String = "res://%s/%s" % [CODEGEN_FOLDER, output_file_name]
	var file = FileAccess.open(output_file_path, FileAccess.WRITE)
	if file:
...
func generate_reducer_gdscript(schema: Dictionary) -> String:
	...
	for reducer in schema.get("reducers", []):
		...
		"\tvar _id_: int = SpacetimeDB.call_reducer('%s', [%s], [%s])\n" % \
		[reducer_name, param_names, param_types] + \
		"\tvar _result_ = await SpacetimeDB.wait_for_reducer_response(_id_)\n" + \
		"\tcb.call(_result_)\n\n"
...
func generate_module_link(modules: Array[String]) -> void:
	...
	for module_name in modules:
		content += "const %s = preload('res://%s/module_%s.gd')\n" % \
		[module_name.to_pascal_case(), CODEGEN_FOLDER, module_name.to_snake_case()]
