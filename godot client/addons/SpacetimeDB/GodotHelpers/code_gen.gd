			...
			if type.has("table_names"):
				...
				for i in table_names.size():
					...
					if HIDE_PRIVATE_TABLES and not type.get("is_public", [])[i]:  
						...
						continue
					generated_table_names.append(table_names[i])
					
			var content: String = generate_struct_gdscript(type, module_name, generated_table_names)
...
func generate_struct_gdscript(type, module_name, table_names) -> String:
