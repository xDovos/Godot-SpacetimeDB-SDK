...
def parse_schema(schema, module_name):
    ...
    for type_info in schema_types:
        ...
        if struct:
            ...
            if type_data.get('gd_native', False) is False:
                TYPE_MAP[type_name] = to_pascal_case(module_name) + type_name
                META_TYPE_MAP[type_name] = to_pascal_case(module_name) + type_name
...
def build_gdscript_from_schema(schema):
    module_name = schema.get("module", None)
    for _type in schema.get("types", []):
        ...
        if struct:
            ...
            if table:
                folder_path = "tables"
            content = generate_struct_gdscript(_type, module_name)
            output_file_name = f"{module_name}_{to_lower_snake_case(_type.get('name'))}.gd"
            ...
            folder_path = "spacetime_types"
            content = generate_enum_gdscript(_type, module_name)
            output_file_name = f"{module_name}_{to_lower_snake_case(_type.get('name'))}.gd"
            output_file_path = os.path.join(OUT_PATH, folder_path, output_file_name)
            try:
                with open(output_file_path, 'w', encoding='utf-8') as f:
                ...
                print_error(f"Failed to write to file {output_file_path}: {e}")
    generate_module_gdscript(schema)

def generate_module_gdscript(schema):
...
class_name {to_pascal_case(module_name)}_Module extends Resource\n\n"""
    for _type in schema.get("types", []):
        if _type.get("gd_native", False): continue
        type_name = _type.get('name')
        subfolder = _type.get('table_name', None)
        if subfolder:
            subfolder = "tables"
        else:
            subfolder = "spacetime_types"
        content += f"""const {to_pascal_case(type_name)} = preload("res://{OUT_PATH}/{subfolder}/{module_name}_{to_lower_snake_case(type_name)}.gd")\n"""
    content += "\n"
...
\tcb.call(result)\n
"""
    output_file_name = f"module_{module_name}.gd"
...
def generate_enum_gdscript(enum_data, module_name):
    ...
    class_name = to_pascal_case(module_name) + enum_name
    
    gd_script_content = f"""#Do not edit this file, it is generated automatically.
class_name {class_name} extends Resource
...
\t\t\treturn "Unknown"\n\n"""
    ...
    gd_script_content += f"""static func create(type: int, _data: Variant = null) -> {class_name}:
...
\treturn result\n\n"""
    ...
    for value in enum_values:
        ...
        if value_type == None:
            gd_script_content += f"""static func create_{to_lower_snake_case(value_name)}() -> {class_name}:
\treturn create({value_name})\n\n"""
            continue
        if value.get('is_array', False):
            value_type = f"Array[{value_type}]"
        gd_script_content += f"""static func create_{to_lower_snake_case(value_name)}(_data: {value_type}) -> {class_name}:
\treturn create({value_name}, _data)\n\n"""

    return gd_script_content

def generate_struct_gdscript(struct_data, module_name):
    struct_name = struct_data.get('name', None)
    fields = struct_data.get('struct', [])
    meta_data = []
    table_name = struct_data.get('table_name', None)
    class_name = to_pascal_case(module_name) + struct_name
...
class_name {class_name} extends Resource\n\n"""
    ...
    gd_script_content += f"""static func create({', '.join("_" + c for c in class_fields)}) -> {class_name}:
\tvar result = {class_name}.new()"""
    for field in fields:
        name = field.get('name', None)
        gd_script_content += f"""\n\tresult.{name} = _{name}"""
...
def generate_main_module_gdscript():
    content = f"""#Do not edit this file, it is generated automatically.
class_name SpacetimeModule extends Resource\n\n"""
    for module in MODULE:
        content += f"""const {to_pascal_case(module)} = preload("res://{OUT_PATH}/module_{module}.gd")\n"""
    file_name = "spacetime_modules.gd"
    try:
        with open(os.path.join(OUT_PATH, file_name), 'w', encoding='utf-8') as f:
            f.write(content)
    except Exception as e:
        print_error(f"Failed to write to file {os.path.join(OUT_PATH, file_name)}: {e}")
...
def main():
    ...
    for mod in MODULE:
        ...
        if download_schema is None:
            print_error("Failed to retrieve schema.\nExiting...")
            exit(1)
        schema = parse_schema(download_schema, mod)
        save_schema(schema)
        build_gdscript_from_schema(schema)
    generate_main_module_gdscript()
